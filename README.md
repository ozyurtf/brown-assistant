...
In addition, retrieval + generation process usually takes between 5-20 seconds. Sometimes it may take up to 30 seconds for the chunks to be retrieved from the vector store and for an answer to be generated. To reduce the latency, Cross Encoder part is currently disabled. But it can be activated by specifying `rerank_top_n` and `rerank_min_score` parameters in the `rag.retrieve()` function in `api.py`. 
